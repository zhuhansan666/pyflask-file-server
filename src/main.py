...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', action='store_true', dest='debug', help='(附加参数, 如果您携带此参数即视为启用, 下均同) 开启 flask 内置的 DEBUG 模式. 如果您不知道这是什么, 请勿附带.')
    parser.add_argument('--no-recursion', action='store_false', dest='recursion', help='是否不递归扫描(即扫描子文件夹), 默认值为 True, 携带本参数后禁用.')
    ...
    if PORT is not None and not (0 <= PORT <= 65535):
        print(f'非法的端口号 {PORT}, 应 0 <= --port <= 65535')
        sys.exit(-1)
    ...
    if DEBUG:
        ...
        os.environ['QUART_ENV'] = 'development'  # 设置 flask 环境为开发
